...
- ✅ **Task Duration**: Extract (1-3m), Enrich (30s), Load (15s), Inference (120s)
- ⚠️ **Retry Count**: Monitor tasks with >1 retry
- ❌ **Failed Tasks**: Investigate immediately

**Common Issues**:

#### **1. Model Not Found**
```
Error: No registered model with name 'stock_predictor_spark_rf_unified_29stocks'
```
**Solution**:
```bash
...
#### **2. Feature Scaler Missing**
```
Error: Feature scaler not found in model artifacts
```
**Solution**: Retrain model with feature scaling enabled in config.

#### **3. Delta Table Not Found**
```
Error: Path does not exist: delta_tables/stock_AAPL
```
**Solution**:
```bash
# Run initial data load
python src/extract_yfinance.py
python src/compute_sentiment_columns.py
python src/load_delta.py
```
...
default_args = {
    "execution_timeout": timedelta(minutes=10),  # Default: 5 min
}
```
...
print(f"Throughput: {29/duration:.2f} stocks/sec")
```
...
null_counts = predictions.select(
    [sum(col(c).isNull().cast("int")).alias(c) for c in predictions.columns]
)
null_counts.show()
...
failure_alert = SlackWebhookOperator(
    ...
    message="❌ Daily stock pipeline failed! Check Airflow logs.",
    trigger_rule="one_failed"
)

# Add to DAG
[extract_task, enrich_task, load_task, inference_task] >> failure_alert
```
...
✅ **Always use versioned models**:
```python
# Good: Specific version
model_uri = "models:/stock_predictor_spark_rf_unified_29stocks/1"

# Avoid: Latest (can change unexpectedly)
model_uri = "models:/stock_predictor_spark_rf_unified_29stocks/Latest"
```
...
### **3. Delta Table Maintenance**
```bash
# Vacuum old files (keep 7 days)
spark.sql("VACUUM delta.`delta_tables/stock_predictions/` RETAIN 168 HOURS")

# Optimize for query performance
spark.sql("OPTIMIZE delta.`delta_tables/stock_predictions/`")
```

### **4. Monitoring**
✅ **Log key metrics**:
- Inference time per stock
...
```python
df.cache()  # Cache Delta table reads
...
**Version**: 1.0.0  
**Production Status**: ✅ Active (Daily at 23:00 UTC)
