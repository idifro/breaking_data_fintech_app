...
- Large datasets (10+ stocks)
- Distributed environments
- Performance-critical applications

⚠️ **Use Spark GBT** for:
- Small datasets only (<5 stocks)
- When interpretability is crucial
- Baseline comparisons

❌ **Avoid Hybrid Sklearn** for:
- Production use (poor accuracy)
- Large datasets (centralization bottleneck)

### **2. Resource Configuration**

**Recommended Spark Config** (20+ stocks):
```yaml
spark:
  driver_memory: 12g
  executor_memory: 10g
...
- Use small subset (4-5 stocks) for tuning
- Apply best params to full dataset
- Re-tune every 3-6 months

### **5. Evaluation**
- Always use train-test split (85/15)
- Monitor multiple metrics (MAE, RMSE, R²)
- Track training time and memory
- Save all results to MLflow

### **6. Production Deployment**
1. Train on full 29 stocks
2. Register model in MLflow
3. Tag as "Production"
4. Version control (v1, v2, etc.)
...
#### **2. Slow Training (Spark GBT)**
- ✅ **Solution**: Switch to Spark RF
- Or reduce maxIter (150 → 100)
- Or reduce maxDepth (12 → 8)

#### **3. Poor Accuracy**
- Check feature engineering (missing sentiment?)
- Verify data quality (nulls, outliers)
- Try hyperparameter tuning
- Increase model complexity (maxDepth, numTrees)

#### **4. MLflow Tracking Issues**
```bash
# Ensure MLflow server is running
mlflow server --backend-store-uri ./mlflow_tracking
...
1. **Spark RF is the clear winner** for production use
   - 5.5x faster than Spark GBT
   - Best accuracy (MAE: 0.0143, R²: 0.0140)
   - Truly distributed training
   
2. **Scalability matters**
   - Spark RF scales best to 29 stocks
   - Memory scales linearly across all modes
   - Time scaling is the key differentiator

3. **Hyperparameter tuning improves accuracy**
   - Best params: depth=12, trees=150, lr=0.01
   - 50 trials with 3-fold CV
   - Test MAE: 0.0176

4. **Production deployment**
   - Model registered in MLflow (v1)
   - Used for daily inference in Airflow pipeline
   - Handles 29 stocks in ~120 seconds
